...
%% 3. 边缘提取模块
% 3.1 Canny边缘检测
% 使用二值图像进行边缘检测，而不是滤波后的图像
[~, thresh] = edge(binary_img, 'Canny');
edges = edge(binary_img, 'Canny', [0.1 0.3]);  % 使用较低的阈值以检测更多边缘
...
circle_edges = bwareaopen(circle_edges, 20);  % 移除小区域

% 3.3 形态学处理优化边缘
se1 = strel('line', 3, 0);    % 水平线
se2 = strel('line', 3, 90);   % 垂直线
se3 = strel('line', 3, 45);   % 45度线
se4 = strel('line', 3, -45);  % -45度线
se5 = strel('disk', 2);       % 圆形结构元素，用于增强圆孔边缘

% 连接断开的边缘
edges = imdilate(edges | circle_edges, [se1 se2 se3 se4 se5]);
edges = bwmorph(edges, 'bridge');  % 连接断开的边缘
edges = bwmorph(edges, 'clean');   % 移除孤立点
edges = bwmorph(edges, 'spur');    % 移除毛刺
edges = bwmorph(edges, 'thin', Inf);  % 细化边缘

% 3.4 使用区域属性过滤小的边缘段
edges = bwareaopen(edges, 30);  % 移除小于30像素的连通区域
